...
public class ProductListFragment extends ListFragment {
    ...
    private class MyListAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup container) {
            if (convertView == null) {
                convertView = LayoutInflater.from(getActivity()).inflate(R.layout.list_item_product, container, false);
            }

            final M_Product item = (M_Product) getItem(position);
            ((TextView) convertView.findViewById(R.id.name)).setText(item.getName());
            ((TextView) convertView.findViewById(R.id.unitprice)).setText(String.format( "%.2f", BigDecimal.valueOf(item.getPriceStd()).movePointLeft(2)));
